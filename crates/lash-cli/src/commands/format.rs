...
mod tests {
    ...
    // --- paths.is_empty() branch disambiguation (mut-000283) ---

    /// With empty paths and an explicit `project_root` that contains an unformatted file,
    /// check mode must return exit 2.  A mutation that negates `args.paths.is_empty()`
    /// would instead use the (empty) paths vec, discover 0 files, and return 0.
    #[test]
    fn test_execute_empty_paths_uses_project_root_in_check_mode() {
        let temp = TempDir::new().unwrap();
        // Put an unformatted file in the temp dir used as project root
        write_needs_formatting_file(&temp, "lash.index.md");
        ...
        // Original: discovers files from project_root → exit 2
        // Mutation (paths NOT empty check): uses empty paths vec → 0 files → exit 0
        assert_eq!(execute(args).unwrap(), 2);
    }

    /// With explicit paths, those specific paths must be processed even if a project root
    /// would contain additional files.  This distinguishes the two branches of the
    /// `if args.paths.is_empty()` condition.
    #[test]
    fn test_execute_explicit_paths_not_expanded_to_project_root() {
        let temp = TempDir::new().unwrap();
        // One file that is already formatted, one that needs formatting
        let formatted = write_already_formatted_file(&temp, "good.md");
        write_needs_formatting_file(&temp, "bad.md");

        // Pass only the already-formatted file explicitly
        let args = FormatArgs {
            paths: vec![formatted],
            check: true,
            ..default_args(vec![])
        };
        // Original: processes only good.md (already formatted) → exit 0
        // If the branch were negated, it would use project_root (temp dir) and find
        // bad.md too → exit 2
        assert_eq!(execute(args).unwrap(), 0);
    }

    // --- files.is_empty() early-return branch (mut-000285) ---

    /// When no files are found the function should return Ok(0) immediately.
    /// A negated `files.is_empty()` would skip the early return and still produce
    /// Ok(0) via the normal path, but the check-mode test above already covers the
    /// symmetric case where the early return must NOT fire when files exist.
    #[test]
    fn test_execute_empty_dir_returns_zero_not_error() {
        let temp = TempDir::new().unwrap();
        let empty_subdir = temp.path().join("subdir");
        fs::create_dir(&empty_subdir).unwrap();

        // No markdown files → files.is_empty() is true → early return Ok(0)
        let result = execute(default_args(vec![empty_subdir]));
        assert_eq!(result.unwrap(), 0);
    }

    // --- json vs text output branch (mut-000286) ---

    /// `json=true` routes through `output_json_results`; `json=false` through `output_text_results`.
    /// The exit code must be the same in both modes, but exercising both paths ensures
    /// the routing condition is not silently inverted.
    #[test]
    fn test_execute_json_false_and_json_true_both_succeed_for_already_formatted() {
        let temp1 = TempDir::new().unwrap();
        let path1 = write_already_formatted_file(&temp1, "lash.index.md");
        let result_text = execute(FormatArgs {
            json: false,
            ..default_args(vec![path1])
        });
        assert_eq!(result_text.unwrap(), 0);

        let temp2 = TempDir::new().unwrap();
        let path2 = write_already_formatted_file(&temp2, "lash.index.md");
        let result_json = execute(FormatArgs {
            json: true,
            ..default_args(vec![path2])
        });
        assert_eq!(result_json.unwrap(), 0);
    }

    /// In check mode, `json=true` must return exit 2 when a file needs formatting,
    /// exactly as `json=false` does.  This locks down the json-routing branch in `execute()`.
    #[test]
    fn test_execute_json_true_check_mode_exits_two_when_needs_formatting() {
        let temp = TempDir::new().unwrap();
        let path = write_needs_formatting_file(&temp, "lash.index.md");
    ...
    // --- error diagnostic classification (mut-000323) ---

    /// When a file cannot be read/parsed, the error diagnostic must include the
    /// "may have syntax errors" help text that is specific to parse failures.
    /// A mutation that negates the `contains("Failed to parse")` check would
    /// produce a diagnostic without that help text.
    #[test]
    fn test_error_diagnostic_for_nonexistent_file_has_failed_count_one() {
        let temp = TempDir::new().unwrap();
        let nonexistent = temp.path().join("ghost.md");
        ...
        // The file cannot be parsed (does not exist), so it is a "Failed to parse" error
        assert_eq!(result.failed, 1);
        assert_eq!(result.error_diagnostics.len(), 1);
    }

    /// The error diagnostic for a non-existent file should NOT carry the
    /// "Failed to write" message — it is a read/parse error, not a write error.
    /// This distinguishes the three branches in the error classification chain.
    #[test]
    fn test_error_diagnostic_message_for_nonexistent_file_does_not_contain_write() {
        let temp = TempDir::new().unwrap();
        let nonexistent = temp.path().join("ghost.md");
        ...
        // A parse/read error must not be misclassified as a write error
        assert!(
            !diag.message.contains("write"),
            "parse error diagnostic should not contain 'write': {}",
            diag.message
        );
    }

    // --- output_text_results branch coverage ---

    /// Verify `output_text_results` returns `Ok(())` in check mode when files need formatting.
    #[test]
    fn test_output_text_results_check_mode_needs_formatting_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 2,
            failed: 0,
            needs_formatting_diagnostics: vec![],
        ...
        assert!(output_text_results(&result, &args, None).is_ok());
    }

    /// Verify `output_text_results` returns `Ok(())` in check mode when all files are properly formatted.
    #[test]
    fn test_output_text_results_check_mode_all_formatted_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 0,
            failed: 0,
    ...
    /// Verify `output_text_results` returns `Ok(())` in format mode when files were formatted.
    #[test]
    fn test_output_text_results_format_mode_formatted_files_returns_ok() {
        let result = FormatResult {
            formatted: 3,
            needs_formatting: 0,
            failed: 0,
    ...
    /// Verify `output_text_results` returns `Ok(())` in format mode when all files were already formatted.
    #[test]
    fn test_output_text_results_format_mode_already_formatted_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 0,
            failed: 0,
    ...
    /// Verify `output_text_results` returns `Ok(())` when some files failed.
    #[test]
    fn test_output_text_results_with_failed_files_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 0,
            failed: 1,
    ...
    /// Verify `output_text_results` returns `Ok(())` in check mode with exactly one file needing
    /// formatting (boundary: `needs_formatting == 1`, not 0).
    #[test]
    fn test_output_text_results_check_mode_exactly_one_needs_formatting_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 1,
            failed: 0,
    ...
    /// Verify `output_text_results` returns `Ok(())` in check mode with failures present
    /// (the `result.failed == 0` else-if branch is skipped).
    #[test]
    fn test_output_text_results_check_mode_with_failures_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 0,
            failed: 2,
    ...
    /// Verify `output_text_results` returns `Ok(())` in format mode with exactly one file formatted
    /// (boundary: `formatted == 1`).
    #[test]
    fn test_output_text_results_format_mode_exactly_one_formatted_returns_ok() {
        let result = FormatResult {
            formatted: 1,
            needs_formatting: 0,
            failed: 0,
    ...
    /// Verify `output_text_results` in format mode when `failed > 0` but `formatted == 0`.
    #[test]
    fn test_output_text_results_format_mode_with_failures_and_no_formatted_returns_ok() {
        let result = FormatResult {
            formatted: 0,
            needs_formatting: 0,
            failed: 3,
    ...
    // --- show_progress boundary conditions (mut-000304 through 000313) ---

    /// With exactly 1 file in non-check, non-diff, non-json mode, `show_progress` is false
    /// (`files.len() > 1` is false).  With mutation `files.len() >= 1`, `show_progress` would be
    /// true for a single file.  The result counters must still be correct either way, so
    /// we verify the formatted count remains accurate.
    #[test]
    fn test_format_files_single_file_formatted_count_correct_without_progress() {
        let temp = TempDir::new().unwrap();
        let path = write_needs_formatting_file(&temp, "lash.index.md");
        let config = LashConfig::default();
        let options = FormatOptions::default();
        // check=false, diff=false, json=false, 1 file → show_progress=false
        let args = default_args(vec![path.clone()]);
        let files = vec![path];
        let result = format_files(&files, &config, &options, &args, None).unwrap();
        assert_eq!(result.formatted, 1);
    ...
    /// With 2 files and `check=true`, `show_progress` must be false regardless of file count.
    /// This exercises the `!args.check` part of the `show_progress` condition.
    #[test]
    fn test_format_files_two_files_check_mode_show_progress_false() {
        let temp = TempDir::new().unwrap();
        let path1 = write_needs_formatting_file(&temp, "file1.md");
        let path2 = write_needs_formatting_file(&temp, "file2.md");
        let config = LashConfig::default();
        ...
        let files = vec![path1, path2];
        let result = format_files(&files, &config, &options, &args, None).unwrap();
        // In check mode both files need formatting
        assert_eq!(result.needs_formatting, 2);
        assert_eq!(result.formatted, 0);
    }

    /// With 2 files and `diff=true`, `show_progress` must be false.
    /// This exercises the `!args.diff` part of the `show_progress` condition.
    /// In diff mode, `format_single_file` detects change (returns true) and since `check=false`
    /// the counter branch `else if changed { formatted += 1 }` fires; so `formatted == 2`.
    #[test]
    fn test_format_files_two_files_diff_mode_show_progress_false() {
        let temp = TempDir::new().unwrap();
        let path1 = write_needs_formatting_file(&temp, "file1.md");
        let path2 = write_needs_formatting_file(&temp, "file2.md");
        let config = LashConfig::default();
        ...
        let files = vec![path1, path2];
        let result = format_files(&files, &config, &options, &args, None).unwrap();
        // In diff mode (not check) the formatted counter increments for changed files
        assert_eq!(result.formatted, 2);
        assert_eq!(result.failed, 0);
    }

    /// With 2 files and `json=true`, `show_progress` must be false.
    /// This exercises the `!args.json` part of the `show_progress` condition.
    #[test]
    fn test_format_files_two_files_json_mode_show_progress_false() {
        let temp = TempDir::new().unwrap();
        let path1 = write_needs_formatting_file(&temp, "file1.md");
        let path2 = write_needs_formatting_file(&temp, "file2.md");
        let config = LashConfig::default();
    ...
    // --- execute() exit code for failed > 0 (mut-000292 through 000297) ---

    /// When a file fails to format (`failed > 0`) and check is false, execute must return
    /// exit code 1 — not 0.  This verifies the `result.failed > 0 → Ok(1)` branch.
    /// We create a file with invalid content that causes a parse failure.
    #[test]
    fn test_execute_exit_code_1_when_file_fails() {
        let temp = TempDir::new().unwrap();
        // Write a file with an invalid checkbox character (triggers a parse error)
        let bad_path = temp.path().join("bad.md");
        fs::write(&bad_path, "- [?] invalid checkbox\n").unwrap();

        let args = default_args(vec![bad_path]);
        let code = execute(args).unwrap();
        assert_eq!(
            code, 1,
            "failed file in non-check mode should produce exit code 1"
        );
    }

    /// When `check=true` AND a file fails (but none need formatting), exit code should be
    /// 1 (failed), not 2 (`needs_formatting`).  The `check && needs_formatting > 0` guard
    /// at the top of the exit-code chain must not fire when `needs_formatting == 0`.
    #[test]
    fn test_execute_exit_code_1_not_2_when_check_mode_with_failure_only() {
        let temp = TempDir::new().unwrap();
        // Write a file with an invalid checkbox character (triggers a parse error)
        let bad_path = temp.path().join("bad.md");
        fs::write(&bad_path, "- [?] invalid checkbox\n").unwrap();
        ...
        let args = FormatArgs {
            check: true,
            ..default_args(vec![bad_path])
        };
        let code = execute(args).unwrap();
        // needs_formatting == 0 (parse failed before formatting could detect a diff);
        // failed == 1 → exit 1, not 2
        assert_eq!(
            code, 1,
            "failure-only result in check mode should produce exit code 1"
        );
    }
}
